--
-- LN-AC Remastered v1.0 by LN (LN-SCRIPTS)
--
...
if LN_AC.Enable then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if LN_AC.AntiInfiniteStamina then
            ...
            if LN_AC.AntiExplosiveBullets then
            ...
            if LN_AC.AntiBlacklistedWeapons then
                for _,_weapon in ipairs(LN_AC.BlacklistedWeapons) do
            ...
            if LN_AC.AntiThermalVision then
                if GetUsingseethrough() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "thermalvision") -- BAN (THERMAL VISION)
                end
                _Wait(300)
            end
            if LN_AC.AntiNightVision then
                if GetUsingnightvision() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "nightvision") -- BAN (NIGHT VISION)
                end
                _Wait(300)
            end
            if LN_AC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
            ...
            if LN_AC.AntiLicenseClears then
            ...
            if LN_AC.AntiBlacklistedTasks then
                for _,task in pairs(LN_AC.BlacklistedTasks) do
            ...
            if LN_AC.AntiBlacklistedAnims then
                for _,anim in pairs(LN_AC.BlacklistedAnims) do
            ...
            if LN_AC == nil or _evhandler ~= AddEventHandler or _onresstarting ~= "onResourceStarting" or _onresstart ~= "onResourceStart" or _onclresstart ~= "onClientResourceStart" or _antistop ~= LN_AC then
    ...
    Citizen.CreateThread(function()
        while LN_AC.AntiVehicleModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if LN_AC.AntiVDM then
                    N_0x4757f00bc6323cfe(-1553120962, 0.0)
                end
                if LN_AC.DisableVehicleWeapons then
    ...
    Citizen.CreateThread(function()
        while LN_AC.AntiWeaponModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
    ...
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(10000)
            local LN_CIT_Wait = Citizen.Wait
    ...
    Citizen.CreateThread(function()
        ...
        while LN_AC.AntiCommandInjection do
            for _,cmd in ipairs(GetRegisteredCommands()) do
                if inTable(LN_AC.BlackListedCMD, cmd.name) then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
        if LN_AC.AntiPedRevive and LN_AC.UseESX then
            local distance = #(LN_AC.HospitalCoords - vector3(GetEntityCoords(PlayerPedId())))
            if distance < 50 then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checkifneargarage", function()
        if LN_AC.AntiVehicleSpawn then
            local _pcoords = GetEntityCoords(PlayerPedId())
            local isneargarage = false
            for _,v in pairs(LN_AC.GarageList) do
    ...
    AddEventHandler('ZRQA3nmMqUBOIiKwH4I5:clearpeds', function()
        if LN_AC.ClearPedsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearprops", function()
        if LN_AC.ClearObjectsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearvehicles", function(vehicles)
        if vehicles == nil then
            ...
            if LN_AC.ClearVehiclesAfterDetection then
                local vehs = GetGamePool('CVehicle')
                for _, vehicle in ipairs(vehs) do
    ...
    AddEventHandler("EuiAtK0QfujTpzWY0Mmp", function(data)
        if data ~= nil then
            exports['LNac-module2']:requestScreenshotUpload(data, 'files[]', function(data2) end)
        end
    end)
    ...
    Citizen.CreateThread(function()
        Citizen.Wait(5000)
        while LN_AC.OnScreenMenuDetection do
            if not ischecking then
                exports['LNac-module2']:requestScreenshot(function(data)
                    Citizen.Wait(1000)
                    SendNUIMessage({
            ...
            Citizen.Wait(LN_AC.OSMDCheckingTime)
        end
    end)
    ...
    RegisterNUICallback('menucheck', function(data)
        if LN_AC.OnScreenMenuDetection then
            if data.text ~= nil then     
                for _, word in pairs(LN_AC.BlacklistedMenuWords) do
    ...
    Citizen.CreateThread(function()
        while LN_AC.CheckPlayersMoney do
            Citizen.Wait(5000)
            local efectivo = nil
            local banco = nil
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        ...
        if _entityowner == GetPlayerServerId(PlayerId()) or args[2] == -1 and LN_AC.AntiAimbot then
        ...
        if LN_AC.DeleteBrokenCars then
        ...
        if LN_AC.AntiSuicide then
            if name == 'CEventNetworkEntityDamage' and args[2] == -1 then
                if LN_AC.usingBuild2189 then
    ...
    if LN_AC.AntiResourceStartorStop then
